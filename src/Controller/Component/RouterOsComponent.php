 ...
 * @category CakePHP3
 * 
 * @package RouterOs\Controller\Component
 * 
 * About RouterOsComponent
 * @author Ivan Amat <dev@ivanamat.es>
 * @copyright Copyright 2016, Iván Amat
 * @license MIT http://opensource.org/licenses/MIT
 * @link https://github.com/ivanamat/cakephp3-routeros
 * 
 * About RouterOS PHP API class v1.6
 * @version RouterOS PHP API class v1.6
 * @author: Denis Basta
 * Contributors:
 *    Nick Barnes
...
namespace RouterOs\Controller\Component;

use Cake\Controller\Component;

class RouterOsComponent extends Component {

    public $debug = false;      //  Write debug information in the debug Log
    ...
    /**
     * Find: A list of items is obtained. If conditions are specified, returns a list of items that match these conditions.
     * 
     * @author Ivan Amat <dev@ivanamat.es>
     * @copyright Copyright 2016, Iván Amat
     * 
     * @param string $path A string with the command path to send. Do not include the action (getall, add, set,... ).
     * @param mixed $potions (int/array) '.id' of element / Array of fields to search as [key => value]
     * @return boolean
     */
    public function find($path, $options = array()) {
        if (!$this->connected) {
            return __('The device seems to be disconnected!');
        }
        ...
        return $this->comm($path."/getall", $query);
    }
    
    /**
     * Get: A specific element is obtained
     * 
     * @author Ivan Amat <dev@ivanamat.es>
     * @copyright Copyright 2016, Iván Amat
     * 
    ...
    public function get($path, $id) {
        ...
        $query = ["?.id" => $id];
        $rows = $this->comm($path."/getall", $query);
     ...
     * 
     * @param string $path A string with the command path to send. Do not include the action (getall, add, set,... ).
     * @param array If set '.id', the element with the '.id' will be updated, otherwise create a new one.
     * @return boolean
     */
    public function save($path, $options = array()) {
        ...
        if(isset($options['id'])) {
            return $this->comm($path."/set", $query);
        }
        
        return $this->comm($path."/add", $query);
    }
     ...
     * 
     * @param string $path A string with the command path to send. Do not include the action (getall, add, set,... ).
     * @param mixed $options (int/array) '.id' of element or array conditions [key => value].
     * @return mixed
     */
    public function delete($path, $options = array()) {
        ...
        for ($i = 0; $i < count($rows); $i++) {
            $this->write($path."/remove", false);
            $this->write("=.id=" . $rows[$i][".id"]);
            $this->read();
        }
